...
import { Circle, Eraser, Move, Pencil, PenTool, RectangleHorizontalIcon, TriangleIcon, TypeIcon } from "lucide-react";
import { Game } from "../draw/Game";
import axios from "axios";
import { BACKEND_URL } from "@repo/backend-common/config";

// add "text"
export type Tool = "circle" | "rect" | "pencil" | "triangle" | "eraser" | "move" | "text";
...
export function Canvas({
  ...
  const [textBox, setTextBox] = useState<{
    visible: boolean;
    id: number;           // row id (may be negative until server confirms)
    x: number; y: number; w: number; h: number;
    value: string;
  }>({ visible: false, id: -1, x: 0, y: 0, w: 0, h: 0, value: "" });

  // accept open/close requests from Game
  const openEditor = useCallback((p: { id: number; x: number; y: number; w: number; h: number; value: string }) => {
    setTextBox({ visible: true, ...p });
    // focus happens on next paint; handled by autoFocus on <textarea>
  }, []);

  const closeEditor = useCallback(() => {
    setTextBox((t) => ({ ...t, visible: false }));
  }, []);

  // commit: send text content back to Game so it can persist + broadcast
  const commitEditor = useCallback((value: string) => {
    if (!game) return;
    setTextBox((t) => ({ ...t, value }));
    game.commitTextValue(textBox.id, value);
    closeEditor();
  }, [game, textBox.id, closeEditor]);

  // When server confirms temp id -> real id during create,
  // Game will re-open the editor if the user is still editing (nothing to do here).
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      // wire the text bridge
      g.bindTextEditor({
        openEditor,
        closeEditor,
        // allow Game to update value if it needs (e.g., remote edit received)
        setValue: (id, value) => {
          setTextBox((t) => (t.visible && t.id === id ? { ...t, value } : t));
        }
      });
      setGame(g);
      return () => { g.destroy(); }
    }
  }, [canvasRef, roomId, socket, openEditor, closeEditor]);
  ...
  useEffect(() => {
    const fetchRoomUsers = async () => {
      try {
        ...
        if (users && users.length > 0) setRoomLeader(users[0]);
        else setRoomLeader(null);
      } catch (error) {
  ...
  // canvas offset for positioning textarea
        ...
        <textarea
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && !e.shiftKey) {
              e.preventDefault();
              commitEditor(textBox.value);
            }
          }}
          style={{
            position: "absolute",
            left: canvasRect.left + textBox.x,
            top: canvasRect.top + textBox.y,
            width: textBox.w,
            height: textBox.h,
            zIndex: 50,
            resize: "none",
            ...
            color: "#fff",
            font: "16px/1.3 Inter, system-ui, sans-serif",
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            ...
            <IconButton onClick={() => setSelectedTool("triangle")} activated={selectedTool === "triangle"} icon={<TriangleIcon />} />
            <IconButton onClick={() => setSelectedTool("eraser")} activated={selectedTool === "eraser"} icon={<Eraser />} />
            <IconButton onClick={() => setSelectedTool("move")}   activated={selectedTool === "move"}   icon={<Move />} />
            <IconButton onClick={() => setSelectedTool("text")}   activated={selectedTool === "text"}   icon={<TypeIcon />} />
          </div>
        </div>

        {/* Room Information / Users / Activity — unchanged */}
        {/* ... keep your existing sidebar code below ... */}
        ...
        >
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {roomUsers.length > 0 ? roomUsers.map((u) => (
              <li key={u.id} style={{ padding: "8px 0", fontSize: "13px", color: "#e0e7ff", display: "flex", alignItems: "center", gap: "8px" }}>
        ...
        >
          <div style={{ padding: "16px", borderBottom: "1px solid rgba(99, 102, 241, 0.2)", flex: "0 0 auto" }}>
            <h3 style={{ margin: 0, fontSize: "12px", fontWeight: "600", color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.5px" }}>
              Activity Logs
            </h3>
          </div>
          <ul style={{ listStyle: "none", padding: "16px", margin: 0, overflowY: "auto", flex: "1 1 auto" }}>
...
export async function getUserLogs(roomId: string) {
  ...
  return data.messages.map((m: any) => {
    ...
    return { userName: m.user.name, shapeType: parsed.shape.type };
  });
}
